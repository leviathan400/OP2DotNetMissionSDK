...
**Historical note**: TechCor documented this in 2019 - the original `DotNetMissionSDK/ReadMe.txt` (now `README.md`, section 7 "BotPlayer") explicitly stated: *"The `BotPlayer` can take over any player, as long as the player is set to human / `GoHuman()`. The bot requires the full player state that `GoHuman()` enables."* We re-derived the requirement empirically while debugging the 256/4096/4096 reads. The original guidance was buried in the project root readme and we only spotted it later.
