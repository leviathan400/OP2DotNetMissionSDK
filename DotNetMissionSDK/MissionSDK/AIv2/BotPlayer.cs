	...
	public class BotPlayer : IBotPlayer
	{
		...
		*/
		...
		public void Update(StateSnapshot stateSnapshot)
		{
			...
			// Periodic status snapshot â€” overwrites logs/BotPlayer_<N>_Status.txt
		...
		// via File.WriteAllText). Swallows IO exceptions â€” status writing failure must
		// never crash the bot.
		private void WriteStatus(StateSnapshot stateSnapshot)
		{
			try
			{
				...
				sb.AppendLine("# BotPlayer " + playerID + " Status â€” " + botType + (p.isEden ? " (Eden)" : " (Plymouth)"));
				...
				sb.AppendLine("BUILDINGS (" + p.numBuildings + " total â€” " + p.numActiveBuildings + " active, " + p.numIdleBuildings + " idle)");
		...
		private void WriteResearchStatus(StateSnapshot stateSnapshot)
		{
			try
			{
				...
				sb.AppendLine("# BotPlayer " + playerID + " Research â€” " + botType + (p.isEden ? " (Eden)" : " (Plymouth)"));
				sb.AppendLine("# Updated " + stamp + " | tick=" + stateSnapshot.time);
				sb.AppendLine();
